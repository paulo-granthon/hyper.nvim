...
function M.generate()
  ...
  if type(options.colors) == 'string' then
    local colors_ok, colors_or_err = pcall(require, 'hyper.colors.' .. options.colors)
    if not colors_ok then
      return error.throw(colors_or_err)
    end
  ...
  if type(options.highlights) == 'string' then
    local highlights_ok, highlights_or_err = pcall(require, 'hyper.highlights.' .. options.highlights)
    if not highlights_ok then
      return error.throw(highlights_or_err)
    end
...
function M.get_color(group_name, color, colors)
  ...
  return false,
    error.throw(
      'hyper.engine.get_color('
        .. group_name
        .. ', '
        .. color
        .. ', ... ) -- Error: unhandled color value `'
        .. color
        .. '`'
    )
end
